...
ridgePsign <- function(S, lambda, sign, target=default.target(S), method="nlm", verbose=TRUE, ...){
	#####################################################################################################
	# 
	# DESCRIPTION:
	# Ridge estimation of the precision matrix with known sign of its off-diagonal elements.
	#
	# ARGUMENTS:
	# -> S          : Sample covariance matrix.
	# -> lambda     : A numeric representing the value of the penalty parameter.
	# -> target     : A target matrix (in precision terms) for Type I ridge estimators.
	# -> sign       : A character indicating the required sign of the off-diagonal elements of ridge precision estimate.
	#                 Must be either: "pos" (positive) and "neg" (negative).
	# -> method     : Which optimization function should be used: "optim" (constrOptim) or "nlm" (nlminb)?
	# -> verbose    : Logical indicator: should intermediate output be printed on the screen? 
	# -> ...        : Additional arguments passed on to either "optim" or "nlm".
	#
	# DEPENDENCIES: 
	# require("rags2ridges")     # functions from package : ridgeP, default.target
	# require("stats")           # functions from package : nlminb, constrOptim
	#    
	# NOTES:
	# ...
	#
	...
	if (is.na(lambda)){ stop("Input (lambda) is not a positive number.") }
	if (lambda <= 0){ stop("Input (lambda) is not a positive number.") }
	if (!(sign %in% c("neg", "pos"))) {  stop("type should be one of {'neg', 'pos'}")   }
	if (!(method %in% c("nlm", "optim"))) {  stop("type should be one of {'nlm', 'optim'}")   }
	if (as.character(class(verbose)) != "logical"){ stop("Input (verbose) is of wrong class.") }

	# construct initial estimator
	Phat <- ridgeP(S, lambda, type="Alt", target)
	if (sign=="pos"){ Phat[Phat < 0] <- 0 }
	if (sign=="neg"){ slh <- diag(Phat); Phat[Phat > 0] <- 0; diag(Phat) <- slh }
	...
	if (method=="optim"){ 	
		...
		if (method=="optim"){
		        if (sign == "pos"){
                        xhat <- constrOptim(theta=x0+10^(-10), f=.armaPenLLreparP, grad=.armaPenLLreparPgrad, method="BFGS", E1=E1, E2=E2, S=S, 
								lambda=lambda, target=target, ui=diag(length(x0)), ci=rep(0, length(x0)), nonzerosR=nonzeros[,1], nonzerosC=nonzeros[,2], ...) 
                }
                if (sign == "neg"){
                        ui <- rep(-1, length(x0))
                        ui[cumsum(c(1, ncol(Phat):2))] <- 1
                        xhat <- constrOptim(theta=x0-10^(-10), f=.armaPenLLreparP, grad=.armaPenLLreparPgrad, method="BFGS", E1=E1, E2=E2, S=S, 
								lambda=lambda, target=target, ui=diag(ui), ci=rep(0, length(x0)), nonzerosR=nonzeros[,1], nonzerosC=nonzeros[,2], ...) 
                }
                if(xhat$convergence != 0){ nonconvergedComp <- nonconvergedComp + 1 }
                Phat <- E1 %*% diag(xhat$par, ncol=length(x0)) %*% t(E2) + E2 %*% diag(xhat$par, ncol=length(x0)) %*% t(E1)                  	    	        
	} 
	        
	if (method=="nlm"){
		if (sign == "pos"){
			xhat <- nlminb(x0, .armaPenLLreparPforNLM, E1=E1, E2=E2, S=S, lambda=lambda, target=target, lower=rep(0, length(x0)), 
					nonzerosR=nonzeros[,1], nonzerosC=nonzeros[,2],...)
		}
                if (sign == "neg"){
			upper <- rep(0, length(x0))
			upper[cumsum(c(1, ncol(Phat):2))] <- 10^10 
    			xhat <- nlminb(x0, .armaPenLLreparPforNLM, E1=E1, E2=E2, S=S, lambda=lambda, target=target, nonzerosR=nonzeros[,1], nonzerosC=nonzeros[,2], upper=upper, ...)
    		}
		if(xhat$convergence != 0){ converged <- TRUE }
			Phat <- E1 %*% diag(xhat$par, ncol=length(x0)) %*% t(E2) + E2 %*% diag(xhat$par, ncol=length(x0)) %*% t(E1)	    			        	    			
		}
	}
	return(Phat)
