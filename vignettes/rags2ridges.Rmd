...
In the absence of such knowledge, the default could be a data-driven diagnal matrix. The function `default.target()` offers some different approaches to selecting this. A good choice here is often the diagonal matrix times the reciprocal mean of the eigenvalues of the sample covariance as entries.


## Graphical ridge estimation? Why not graphical Lasso?

The graphical lasso (gLasso) is equivalent to graphical ridge, why not perform LASSO?



### Gaussian graphical modeling and post processing

> What is so interesting with the precision matrix anyway? I'm always interested in correlations and the correlation matrix. 

As you may know, correlation does not imply causation.
